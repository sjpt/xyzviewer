...
const OrbitControls = function ( object, domElement ) {
	...
	// Set to true to disable this control
	this.noPan = false;
	this.keyPanSpeed = 2.0;	// pixels moved per frame while key down
    ...
    this.usekeys = function() {
        ...
        if (keysdown['A']) { scope.pan( scope.keyPanSpeed * scope.modSpeed, 0, 0); scope.update(); }
        if (keysdown['D']) { scope.pan( -scope.keyPanSpeed * scope.modSpeed, 0, 0); scope.update(); }

        if (keysdown['I']) { p1(scope.rotateUp, d); }
        if (keysdown['K']) { p1(scope.rotateUp, -d); }
        if (keysdown['L']) { p1(scope.rotateLeft, -d);	}
        if (keysdown['J']) { p1(scope.rotateLeft, d); }

	};

	// p1 makes a operation (fun) work in first person mode rather than fixed target mode
	function p1(fun, d) {
			var p = scope.object.position;
			f.set(p.x,p.y,p.z); 
			fun(d); 
	...
	var f = new THREE.Vector3;  // working intermediate
