...
async _dataToMarkers(pfilterfun = E.filterbox.value, popping, cbs) {
    this.setupGraphics(this.fid);  // for call from pdbReader, or to check incremental sizes
...
setupGraphics(fid = this.fid) {
    ...
    if (!this._verta || this._verta.count != l*3) {    // first time in, or changed during incremental load/display
