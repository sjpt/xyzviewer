...
`;
  ...
  var e, rg, gr, kr, kg, kb, sss, deltaRGB, eqcols;
...
void main() {
  ...
  async function useXShader(pcols = [], id, xyz = X2.currentXyz) {
  ...
  function xclick(col, row) {
    ...
    for (let col2 = 0; col2 < ND; col2++) {
      for (let row2 = 0; row2 < ND; row2++) {
        if (!getm(col2, row2)) setm(col2, row2, void 0);
      }
    }
    for (let col2 = 0; col2 < ND; col2++) {
      for (let row2 = 0; row2 < ND; row2++) {
        if (getm(col2, row2)) {
          for (let cc = 0; cc < ND; cc++)
            if (cc !== col2) setm(cc, row2, 0);
          for (let rr = 0; rr < ND; rr++)
            if (rr !== row2) setm(col2, rr, 0);
        }
      }
    }
    usecheck();
  ...
  function setobj(to, from) {
    for (const x in to) delete to[x];
    Object.assign(to, from);
  }
  var WA, X2, math, xmat, me, MM, ND, uniforms, shader, lastid, tumblerotl, tumblerotr, mouserot, lastxmat, checklist, cols, vmap, attribs, attset, tumblem, lastx, lasty, makeshowxmatdone;
  var init_xshader = __esm({
    "xshader.js"() {
      "use strict";
      init_threeH();
      init_xyz();
      ...
      MM = class _MM {
        ...
        dotrow(r1, r2) {
          const o2 = this.m;
          let s = 0;
          for (let k = 0; k < ND; k++) s += o2[r1 * ND + k] * o2[r2 * ND + k];
          return s;
        }
        ...
        randrow(row, d = 0.1) {
          const o2 = this.m;
          for (let col = 0; col < ND; col++)
            if (getm(col, row) === void 0) o2[row * ND + col] += (Math.random() - 0.5) * d;
        }
        ...
        toOrth(f = 1) {
          ...
          for (let row = 0; row < ND; row++) {
            ...
            for (let k = 0; k < ND; k++) o2[row * ND + k] *= sf;
      ...
      attribs = "";
      for (let i = 0; i < ND; i++) attribs += `attribute float field${i};
`;
      attset = "";
      for (let i = 0; i < ND; i++) attset += `rv[${i}] = field${i};
`;
      tumblem = new MM().random();
      lastx = Infinity;
      makeshowxmatdone = false;
    }
  });
  ...
  function csvReader(data2, fid) {
    return new XYZ(data2, fid);
  }
  var E2, X3, XLSX, binnop, TData, _kkk, _iii, _bbb, iNaN, _baseiNaN, NaN4null;
  var init_tdata = __esm({
    "tdata.js"() {
      "use strict";
      init_basic();
      init_xyz();
...
`;
      ...
      TData = class _TData {
        static tdatas = {};
        static tellUpdateInterval = 1e4;
        // inform update insterval during long load
        static firstUpdate = 1e4;
        static graphicsUpdateInterval = 25e4;
        // reminder, useful regexp for wrongly used _ fields:  (?<![this|me])\._
        /**
             * @param {any} data
             * @param {string} fid
             */
        constructor(data2, fid) {
          this.fid = fid;
          this.pendread = {};
          this.xyzs = [];
         ...
         * @returns {TData}
         */
        static get(data2, fid, xyz) {
          let tdata = data2 instanceof _TData ? data2 : data2 && data2.tdata || _TData.tdatas[fid];
          if (!tdata) tdata = new _TData(data2, fid);
        ...
        async yamlReader(raw, fid) {
          ...
          const y = X3.jsyaml.safeLoad(yaml);
        ...
        async csvReader(raw, fid) {
          ...
          const me2 = this;
          if (oldparse) {
            X3.currentThreeObj = X3.currentXyz = this;
            let sep;
            const st = Date.now();
            let byteLength;
            const linex = function linex2(row, numLines, bytesProcessedSoFar, bytesReadSoFar, length) {
              ...
              if (me2.n % _TData.tellUpdateInterval === 0) {
                const dt3 = ((Date.now() - st) / 1e3).toFixed();
                E2.msgbox.innerHTML = `reading file ${fid}, line ${me2.n}, bytes ${bytesProcessedSoFar} of ${length}, ${(bytesProcessedSoFar / length * 100).toFixed()}%, ${dt3} secs`;
              }
              if (me2.n % _TData.graphicsUpdateInterval === 0 || me2.n === _TData.firstUpdate) {
                log("reading", E2.msgbox.innerHTML);
                me2.finalize(fid, true);
              }
            };
            if (raw instanceof File) {
              console.time("oldparsestream");
              await fileReader(raw, lineSplitter((line, numLines, bytesProcessedSoFar, bytesReadSoFar, length) => linex(line, numLines, bytesProcessedSoFar, bytesReadSoFar, length)));
            ...
            E2.msgbox.innerHTML = `read ${fid} lines ${me2.n}, bytes ${byteLength}, ${dt2} secs`;
        ...
        rebase(fn) {
          ...
          for (let i = 0; i < col.length; i++) col[i] -= c2;
        ...
        genstats(name = void 0) {
          ...
          const sd2 = Math.sqrt((sum2 - 1 / n * sum * sum) / n);
          Object.assign(
            this.ranges[name],
            { name, mean: sum / n, sd: sd2, mid: (min + max) / 2, range: max - min, min, max, sum, sum2, n }
          );
        ...
        async savefiles(yamlonly = false) {
          ...
          var yaml = X3.jsyaml.safeDump(obj, { skipInvalid: true });
      ...
      _kkk = new Float32Array([NaN]);
      _iii = new Uint32Array(_kkk.buffer);
      _bbb = new Uint8Array(_kkk.buffer);
      iNaN = _iii[0];
      _baseiNaN = iNaN + 16;
      NaN4null = iNaN + 1;
    }
...
`;
      ...
      */
      function(data2, fid) {
        ...
        let coldata;
        if (data2[0] === "{")
          coldata = JSON.parse(data2);
        else
          coldata = jsyaml.safeLoad(data2);
      ...
      */
      function(toset, tdata, field) {
        try {
          field = field.trim();
          if (field === "random") return toset + ".set(COLS.random())";
          if (field === "fixed") field = E3.colourpick.value;
          const range = tdata.ranges[field];
          if (range && range.numStrs > range.numNum && !COLS[field]) COLS.autocol(tdata, field);
          if (COLS[field]) return `${toset}.set(COLS['${field}'][xyz.tdata.val('${field}', i)])`;
      ...
      */
      ...
      COLS.writer = async function(fid = "test") {
        ...
        const d = {};
        const ns = X4.currentXyz.tdata.vseti;
      ...
      COLS.show = function(tdata = X4.currentXyz, field = X4.currentXyz.guiset.colourby) {
        const range = tdata.ranges[field];
      ...
      COLS.set = function(f, fixed) {
        if (E3.xshaderbox.checked) {
          X4.currentXyz.setField("MD", f);
          useXShader();
          return;
        }
        if (E3.colourby.value !== f) E3.colourby.value = fixed ? "fixed" : f;
        const ofilt = "\n" + E3.filterbox.value + "\n";
        let g = ofilt.match(/^(.*)\nCOL:(.*?)\n(.*)/s);
        if (g)
...
void main() {
    ...
    const shader3 = new THREE.ShaderMaterial(
      { vertexShader, fragmentShader, uniforms: uniforms3 }
    );
    return shader3;
  }
  function photoShader_clicked(evt2) {
    const ele = evt2 ? evt2.srcElement : E4.photoscheck;
    const particles = X5.currentXyz.particles;
  ...
  var E4, X5, photos, nophotomaterial, photomaterial;
  var init_photoshader = __esm({
    "photoshader.js"() {
      "use strict";
      init_threeH();
      ({ E: E4, X: X5 } = window);
  ...
  var X6, startx, lastx2, starty, lasty2, sv3, Lasso;
    ...
    "lasso.js"() {
      ...
      Lasso = class {
        ...
        mousemove(e2) {
          ...
          if (e2.ctrlKey || e2.buttons === 4) type = "xor";
          else if (e2.altKey || e2.buttons === 2) v = 0;
...
void main() {
    ...
    shader2 = new THREE.ShaderMaterial(
      { vertexShader, fragmentShader, uniforms: uniforms2 }
    );
    shader2.name = "lasso";
    return shader2;
  }
  async function useLassoShader(cols2, id, xyz = X7.currentXyz) {
    const tdata = xyz.tdata;
    if (cols2 === true) cols2 = [xyz.getField("X") || "cd3", xyz.getField("Y") || "cd4", xyz.getField("Z") || "cd16"];
    const particles = xyz.particles;
    if (cols2) {
  ...
  var X7, uniforms2, shader2, lastid2;
  var init_lassoshader = __esm({
    "lassoshader.js"() {
      "use strict";
      init_threeH();
      init_xyz();
  ...
  function onMouseMove(event) {
    if (event.target !== E5.xyzcanvas || !(event.ctrlKey || event.type === "click")) return;
  ...
  var E5, X8, raycaster, mouse, lastface, lastcol, lastint0;
  ...
  // speech.js
  var log2, msgfix, SpeechRecognition, OrganicSpeechF, OrganicSpeech;
  var init_speech = __esm({
    "speech.js"() {
      log2 = console.log;
      msgfix = console.log;
      SpeechRecognition = SpeechRecognition || window.webkitSpeechRecognition;
      OrganicSpeechF = function() {
        let me2 = this;
        let recognition;
        function init() {
          ...
          recognition.onerror = (event) => {
            if (event.error !== "no-speech") log2("recognition error", event.error);
          };
          ...
          recognition.onresult = function(event) {
            ...
            for (const results of event.results) {
              if (results.isFinal && newsoundt) {
                log2("newsound to final", Date.now() - newsoundt);
                newsoundt = 0;
              }
              if (!results.isFinal && !newsoundt) {
            ...
            if (ll.length > 2 || st === 0) log2(dt2, ll.join("\n"));
          };
        }
        me2._running = false;
        me2._start = function() {
          if (me2._running) return;
          if (!recognition) init();
          recognition.start();
          me2._running = true;
        };
      ...
      OrganicSpeech = new OrganicSpeechF();
  ...
  var E6, zoomDelta, mode, rate, panrate, o, c, vrmat, imat, mat3, v3;
  var init_xyzspeech = __esm({
    "xyzspeech.js"() {
      init_speech();
      init_graphicsboiler();
      init_threeH();
...
`;
  ...
  function refit() {
    ...
    for (let i = 0; i < refitcol.length; i++) {
      ...
      let gr2 = rgroups[gid];
      if (!gr2) gr2 = rgroups[gid] = { gid, sx: 0, sy: 0, sz: 0, n: 0, inds: [] };
      gr2.sx += xc[i];
      gr2.sy += yc[i];
      gr2.sz += zc[i];
      gr2.n++;
  ...
  var X9, rgroups, rlines;
  var init_refit = __esm({
    "StarCarr/refit.js"() {
      "use strict";
      init_graphicsboiler();
      init_xyz();
  ...
  var archstart_exports = {};
  __export(archstart_exports, {
    filelist: () => filelist
  });
  var E7, GG, filelist, starturi, archxref, archhh;
  var init_archstart = __esm({
    "StarCarr/archstart.js"() {
      "use strict";
      init_basic();
      init_graphicsboiler();
...
ta0281_DSM_2M.asc`.trim().split("\n");
      ...
      starturi = "StarCarr/Flint.csv";
      (async function start() {
        await posturiasync(starturi);
        ggb().plan();
        refit();
        await posturiasync("StarCarr/contours.geojson");
        E7.msgbox.innerHTML = "StarCarr load time: " + (Date.now() - GG.starttime);
      })();
      archxref = `<a href="https://archaeologydataservice.ac.uk/archives/view/postglacial_2013/index.cfm" target="_blank">
...
`;
      E7.ack.innerHTML = archhh;
      setTimeout(() => E7.ack.style.display = "none", 1e4);
    }
  });

  // plugins/pdbreader.js
  function pdbReader(data2, fid) {
    ...
    const format = [
      ...
      [7, 11, "atid"],
      //    Atom serial number    right    integer
      [13, 16, "atname"],
      //    Atom name    left*    character
      [17, 17, "altloc"],
      //    Alternate location indicator        character
      [18, 20, "resname"],
      // §    Residue name    right    character
      [22, 22, "chain"],
      ...
      [61, 66, "tempfac"],
      // Temperature factor    right    real (6.2)
      //[73,76,    'segid'], // Segment identifier¶    left    character
      //[77,78, 'elesym'], //     Element symbol    right    character
      [99, 99, "reslen"],
      // length of residue, will be computed later, this reserves data slots
      [99, 99, "resstart"],
      // id for first element in residue, will be computed later, this reserves data slots
      [99, 99, "atinres"]
      // id of atom within reside, computed later
    ];
    ...
    E8.colourby.innerHTML += `<option value="pdbcol">pdbcol</option>`;
    tdata.finalize(fid);
  ...
  async function makechainlines(pfilterfun = E8.filterbox.value, maxd2 = 800) {
    if (chainlines && chainlines.visible === false) return;
    const tdata = myxyz.tdata;
    if (!tdata.fvals) return;
    const filterfun = await myxyz.makefilterfun(pfilterfun, E8.filterbox);
    ...
    for (let i = 0; i < tdata.n - 1; i++) {
      ...
      if (filterfun) {
        if (!filterfun(myxyz, i, tdata.fvals)) continue;
      }
      const col1 = myxyz._col.clone();
      myxyz._col.setRGB(0.3, 0.3, 0.3);
      if (filterfun) {
        if (!filterfun(myxyz, i + 1, tdata.fvals)) continue;
      }
  ...
  var E8, chainlines, myxyz;
  var init_pdbreader = __esm({
    "plugins/pdbreader.js"() {
      "use strict";
      init_graphicsboiler();
      init_xyz();
  ...
  function folddemofun(tt = 1e4, gap = 2e3) {
    ...
    function foldframe() {
      ...
      if (t < tt) {
        const dt2 = t / tt;
        expandchain(0.5 + 0.5 * dt2, 0.5 - 0.5 * dt2);
      } else if (t > tt + gap) {
        const dt2 = (t - tt - gap) / tt;
        expandchain(1 - dt2, 0);
      }
    ...
    for (let i = 0; i < virchains.length; i++) {
      ...
      cl.forEach((j) => {
        ...
        cl.forEach((k) => {
          ...
          let chainset = /* @__PURE__ */ new Set([i, j, k]);
          ...
          switch (ck) {
            ...
            case "303049":
              return;
            // col=cols[30]; break;  // ignore wide triangle from pentagon, reduce overcoverage
            case "424242":
              chi.trimer = chj.trimer = chk.trimer = chainset;
              col = cols2[42];
              break;
            // this is a trimer (red)
            case "303442":
              col = cols2[49];
              break;
            default:
  ...
  function virusshow() {
    E9.colourby.value = "fixed";
    if (ggb().usePhotoShader) {
      myxyz2.setPointSize(2);
      E9.colourby.value = "random";
    }
    dataToMarkersGui();
  }
  var E9, folddemo_st, virchains, polygonmesh, rlines2, groupgeom, myxyz2, opos;
  var init_folddemo = __esm({
    "extras/folddemo.js"() {
      "use strict";
      init_xyz();
      init_basic();
  ...
  function getStartdata() {
    const wls2 = window.location.search || "";
    let r = "";
    if (wls2.startsWith("?ox")) {
      if (location.host.startsWith("csynth") || queryVariables.remote) {
        ...
        if (wls2.startsWith("?ox7m"))
          r = "COVID19_CyTOF/_Steve_UMAP3_allcells.txt.yaml";
        else if (wls2.startsWith("?oxm"))
  ...
  async function showfirstdata() {
    if (location.href.indexOf("xyz.html") === -1) return;
    const wls = window.location.search;
    if (wls.startsWith("?arch")) await Promise.resolve().then(() => (init_archstart(), archstart_exports));
    if (wls.startsWith("?fold")) await Promise.resolve().then(() => (init_folddemo(), folddemo_exports));
  ...
  async function posturiasync(puri, callb, data2 = "") {
  ...
  async function openfile(file, item) {
    ...
    if (handler && handler.rawhandler) {
      ...
      reader.onload = function(e2) {
        var data2 = e2.target.result;
        console.timeEnd("load file: " + file.fullPath);
        log("load", file.name, data2.length);
        handler(data2, file.fullPath);
      };
  ...
  async function _scanFiles(item, availableFileList2 = {}, directoryEntries = {}) {
    return new Promise((resolve) => {
      if (item.isDirectory) {
        ...
        let getEntries = async function() {
          directoryReader.readEntries(async function(entries) {
            ...
            for (const entry of entries) {
              await _scanFiles(entry, availableFileList2, directoryEntries);
            }
            getEntries();
          });
        };
        ...
        availableFileList2[item.fullPath] = item;
        resolve();
      }
    });
  }
  ...
  function addToFilelist(fullPath, fileEntry, displayName) {
    E10.filedropbox.innerHTML = E10.filedropbox.innerHTML.replace("none available", "none selected");
    displayName = displayName || (typeof fileEntry === "string" ? fileEntry : fileEntry.name);
    E10.filedropbox.innerHTML += `<option value="${fullPath}" title="${fullPath}">${displayName}</option>`;
  ...
  function docdragover(evt2) {
    evt2.dataTransfer.dropEffect = "copy";
    return killev(evt2);
  }
  ...
  function saveData(fileName, ...data2) {
  ...
  function fireev(type, data2) {
    xx.dispatchEvent({ type, data: data2 });
  }
  ...
  var E10, X10, GG2, queryVariables, readyFiles, fileTypeHandlers, log, lastdroptarget, availableFileList, selectableFileList, xx;
...
`;
      ...
      if (!window.E) window.E = /**@type{any}*/
      window;
      ({ E: E10, X: X10 } = window);
      ({ GG: GG2 } = window);
      GG2.cols = COLS;
      GG2.ps = photoshader_exports;
      GG2.tdata = tdata_exports;
      GG2.xyz = xyz_exports;
      GG2.basic = basic_exports;
      GG2.lasso = lasso_exports;
      GG2.lassoshader = lassoshader_exports;
      GG2.xshader = xshader_exports;
      GG2.raycast = raycast_exports;
      GG2.gb = graphicsboiler_exports;
      GG2.expose = () => {
        for (const f in GG2) Object.assign(window, GG2[f]);
      };
      GG2.ospeech = OrganicSpeech;
      GG2.xyzspeech = xyzspeech_exports;
      queryVariables = {};
      readyFiles = {};
      log = function() {
      ...
      E10.fileChooser.onclick = function(evtp) {
        this.value = null;
      };
      E10.fileChooser.onchange = function(evtp) {
        openfiles(evtp.target);
      };
      availableFileList = {};
      selectableFileList = {};
      xx = new THREE.EventDispatcher();
      (async function() {
        GG2.test = () => {
    ...
    "jsdeps/OrbitControls.js"() {
      ...
      OrbitControls = function(object, domElement) {
        ...
        function onKeyDown(event) {
          ...
          if (event.keyCode === 27) scope.keysdown = {};
  ...
  var THREEX, TrackballControls;
  var init_TrackballControls = __esm({
    "jsdeps/TrackballControls.js"() {
      init_threeH();
      init_graphicsboiler();
      ({ THREE: THREEX } = window);
  ...
  function vrframe() {
    ...
    let d = panDelta;
    if (igp1 && igp1.axes && igp1.buttons[2].pressed) d *= -igp1.axes[1];
    else if (!select) d = 0;
  ...
  var controller1, controller2, select;
  var init_vrcontrols = __esm({
    "vrcontrols.js"() {
      init_graphicsboiler();
      select = false;
    }
  ...
  function start2() {
    ...
    E11.lastmod.textContent = window.lastModified.xyzhtml;
    if (location.href.indexOf("xyz.html") !== -1) {
      _ggb = new GraphicsBoiler();
      vrstart();
    }
  }
  var E11, X11, Stats, gbid, _ggb, GraphicsBoiler;
  var init_graphicsboiler = __esm({
    "graphicsboiler.js"() {
      "use strict";
      init_basic();
      init_VRButton();
...
`;
      ...
      ({ E: E11, X: X11, Stats } = window);
      gbid = 0;
      GraphicsBoiler = class _GraphicsBoiler {
        constructor(id = "gb" + gbid++) {
          ...
          this.canvas.onclick = () => {
            ...
            if (Object.values(av).length !== 0) {
              ...
              if (!X11.currentXyz) X11.currentXyz = Object.values(av)[0].obj.xyz;
            }
            _ggb = me2;
            if (X11.currentXyz) console.log("click", X11.currentXyz.fid, me2.id);
          };
          if (Stats) {
        ...
        static xyzspeechupdate;
        // called each frame for speech control. ? todo arrange event mechanism
        ...
        render() {
          ...
          if (this.orbitControls) {
            ...
            if (_GraphicsBoiler.xyzspeechupdate) _GraphicsBoiler.xyzspeechupdate();
          }
        ...
        onDocumentKeyDown(evt2) {
          const k = String.fromCharCode(evt2.keyCode);
        ...
        fullcanvas(full = E11.info.style.display !== "none") {
          E11.info.style.display = full ? "none" : "";
          E11.ack.style.display = E11.info.style.display;
          this.canvas.focus();
        }
    ...
    `;
        ...
        select(fid, xyz) {
          ...
          for (const f in this.addvisList)
            E11[f + "_k"].style.color = f === fid ? "lightgreen" : "white";
          const avl = this.addvisList[fid];
          log("sselect", fid, avl);
          if (avl) X11.currentThreeObj = avl.obj;
          xyz = xyz || avl.xyz || avl;
          if (xyz) {
            X11.currentXyz = xyz;
            const guiset = xyz.guiset;
            if (guiset) {
              E11.colourby.value = guiset.colourby;
              E11.filterbox.xvalue = guiset.filterbox;
              E11.colourpick.value = guiset.colourpick;
  ...
  function filterAdd(s, end = false) {
    const l = E12.filterbox.value.split("\n");
    if (l.includes(s)) return;
    if (end)
      l.push(s);
    else
      l.unshift(s);
    E12.filterbox.xvalue = l.join("\n");
    dataToMarkersGui();
  }
  function filterRemove(s) {
    let l = E12.filterbox.value.split("\n");
    l = l.filter((ll) => ll !== s);
    E12.filterbox.xvalue = l.join("\n");
    dataToMarkersGui();
  }
  ...
  var E12, X12, setPointSize, filtergui, dataToMarkersGui, centrerange, badfun, usePhotoShader, XYZ;
  var init_xyz = __esm({
    "xyz.js"() {
      init_graphicsboiler();
      init_basic();
...
`;
      ...
      ({ E: E12, X: X12 } = window);
      setPointSize = (a, b) => {
        if (X12.currentXyz) X12.currentXyz.setPointSize(a, b);
      };
      filtergui = (g) => {
        if (X12.currentXyz) X12.currentXyz.filtergui(g);
      };
      dataToMarkersGui = (type, popping) => {
        if (X12.currentXyz) X12.currentXyz.dataToMarkersGui(type, popping);
      };
      centrerange = new THREE.Vector3(Infinity);
      badfun = () => -9999;
      XYZ = class _XYZ {
        ...
        constructor(pdata, fid, owngb) {
          ...
          if (Object.keys(tdata.xyzs).length === 0)
            this.guiset.filterbox = E12.filterbox.value;
          this.gb.select(fid, this);
          if (pdata)
            this.finalize(fid);
          if (_XYZ.constructorDone) _XYZ.constructorDone(this);
        ...
        dataToMarkersGui(type = void 0, popping = false) {
          ...
          if (E12.xshaderbox.checked) {
            useXShader();
            return;
          }
          if (X12.currentThreeObj.xyz) {
            if (type) E12.colourby.value = type;
            if (!this.guiset) this.guiset = _XYZ.baseguiset;
            Object.assign(this.guiset, { colourby: E12.colourby.value, filterbox: E12.filterbox.value, colourpick: E12.colourpick.value });
            if (this.makechainlines)
              this.makechainlines(E12.filterbox.value);
            return this.dataToMarkers(E12.filterbox.value, popping);
          } else if (X12.currentThreeObj.material) {
            X12.currentThreeObj.material.color.set(E12.colourpick.value);
          }
        }
        /**
        ...
        /** 
         make sure the columns to be used for data are ready, and have (at least tentative) statistics
         */
        _prepcols() {
        ...
        async _dataToMarkersFast() {
          ...
          const dt2 = performance.now() - st;
          E12.filtcount.innerHTML = `fast filter applied: #points=${l} of ${tdata.n}, time: ${dt2}ms`;
          log(E12.filtcount.innerHTML);
          E12.filterbox.classList = ["_fast"];
         ...
         * @param {[any]} cbs ?? 
         */
        async _dataToMarkers(pfilterfun = E12.filterbox.value, popping, cbs) {
          ...
          const filterfun = await this.makefilterfun(pfilterfun, E12.filterbox, "force");
          tdata.showpendread();
          ...
          for (let i = 0; i < pendl; i++) {
            if (me2._ids !== void 0 && !me2._ids[did[i]]) continue;
            q[0] = q[3] = NaN;
          ...
          const dt2 = Math.round(performance.now() - st);
          if (cbs) return;
          ...
          if (filterfun) {
            if (lines)
              E12.filtcount.innerHTML = `filter applied: #lines=${ll} of ${pendl} of ${fulll}, time: ${dt2}ms`;
            else
              E12.filtcount.innerHTML = `filter applied: #points=${ll} of ${pendl} of ${fulll}, time: ${dt2}ms`;
          } else if (pfilterfun) {
            E12.filtcount.innerHTML = `bad filter not applied`;
            ok = false;
          } else {
            E12.filtcount.innerHTML = `no filter applied: #points=${pendl} of ${fulll}, time: ${dt2}ms`;
          }
          log(E12.filtcount.innerHTML);
          if (ok && !popping && location.href.indexOf("xyz.html") !== -1) {
            let ll2 = location.href.split("&control=")[0];
            if (ll2.indexOf("?") === -1) ll2 += "?";
            if (pfilterfun)
              ll2 += "&control=" + pfilterfun.split("\n").join("!!!");
            if (ll2 !== location.href)
              history.pushState({}, void 0, ll2);
          }
          await this.makefilterfun(pfilterfun, E12.filterbox, "confirm");
        }
        ...
        async makefilterfun(filtin, box, mode2 = "") {
          ...
          let filt = filtin;
          const msg = (m, col) => {
            E12.filterr.innerHTML = `${m} <br><code> ${filt.split("\n").join("<br>")}<code>`;
            if (box) box.classList = [col];
            E12.filterr.classList = [col];
          };
          if (mode2 === "force") this.lastInputApplied = void 0;
...
return q;`;
        ...
        setPointSize(eventsize, temp = "") {
          ...
          if (temp === "in" && size * size * this.tdata.n > 4e5) return;
          ...
          if (usePhotoShader) {
            ...
            if (size !== void 0) sizeUniform.value = size * k;
            return r;
          }
          if (size !== void 0 && this.material) this.material.size = size;
          this.guiset.spotsize = size;
          const radio = E12["spot" + this.guiset.spotsize];
        ...
        async filtergui(evt2 = {}) {
          const box = E12.filterbox;
          const filterr = E12.filterr;
          const boxv = box.value.trim();
          try {
            const fun = await this.makefilterfun(boxv, box);
            if (fun === badfun) return;
            if (evt2.keyCode === 13 && evt2.ctrlKey) {
              this.dataToMarkersGui();
            }
          } catch (e2) {
...
`;
        ...
        setField(fieldRole, fieldName, update = true) {
          ...
          if (fieldRole === "MD") {
            ...
            E12.filterbox.xvalue = ofilt + "MD:" + fieldName;
          } else {
            ...
            if (g)
              E12.filterbox.xvalue = `${g[1]}
${fieldRole}:${fieldName}
${g[3]}`.trim();
            else
              E12.filterbox.xvalue = `${ofilt.trim()}
${fieldRole}:${fieldName}`.trim();
        ...
        getField(fieldRole) {
          ...
          const m = E12.filterbox.value.match(filt);
          if (m) return m[1].trim();
          const v = this.def[fieldRole];
          E12.filterbox.xvalue = `${E12.filterbox.value}
${fieldRole}:${v}`;
          return v;
        }
        ...
        setHostDOM(host) {
          ...
          renderer.domElement.ondblclick = () => E12.xyzviewergui.style.display = E12.xyzviewergui.style.display ? "" : "none";
        }
        getHostDOM() {
